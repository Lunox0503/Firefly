...
// 构建时跳过联网抓取，只复用本地快照。
// 用途：友链里只要有一个站点在国内网络/代理下不可达，构建就会卡在超时重试上，
// 加上这个开关可以让构建离线、快速且可复现：
//   FRIENDS_FEED_OFFLINE=1 pnpm build
// 默认不开启，保持和原主题一致的行为（CI 定时任务照常刷新快照）。
const FRIENDS_FEED_OFFLINE = process.env.FRIENDS_FEED_OFFLINE === "1";
		...
		try {
			const response = await fetch(url, {
				...
				headers: {
					...
					"User-Agent": "Lunox-Moments/1.1 (+https://lunoxblog.com/)",
				},
			});
			if (response.ok) {
	...
	const records = await mapWithConcurrency<FriendLink, FeedRecord>(friends, 5, async (friend) => {
		if (FRIENDS_FEED_OFFLINE) {
			...
			return cached?.items.length
				...
				: { friend, feedUrls: [], items: [], stale: false };
		}
